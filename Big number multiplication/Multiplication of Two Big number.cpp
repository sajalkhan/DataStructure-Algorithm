...
-->49. int sum=d[j]*e[i]+carry; ai khane Mi(i) er proti ta value er jonno Ma(j) proti ta value gun korci

	50. if((c[i+j]+sum)>9) ai khane c[i+j] bolte i=0,1,2...... r j=0,1,2,..... c[0+1] mane array er c[1] tomo inder niye kaj korci..
...
-->52. int cr=c[i+j]+sum; er khane cr er vitor a tader jog fol ta rekhe dibo

-->53. c[i+j]=(c[i+j]+sum)%10; suppose ai khane array er c[0+1] tomo inder er moddhe jodi amra value nite chai tokhon c[0+1]=(c[0+1] tomo index a j value ache
...
-->54. if(!i)carry=sum/10; jokhon Mi value er 1st index er jonno value amra array te rakhbo tokhon shudu sum k 10 dara vag kore carry ber korbo karon amra ..
...
-->56. if(j==Ma-1)Ma++; jodi j er value akdom last index a ase jemon 	 9
																		x9
																	   ----
																		 1 r 8 but loop kintu akbari gurbe tai oi 8 print korar jonno Ma er man r ek barye diye oita
																		 array er moddhe niye nibo

-->63. if(i==Mi-1) count++; i means Minimun value jokhon ses index a chole jabe tokhon oi index joto gula value pabo oi gula hocce amader final result er digit
	ai digit gula count kore print korte hobe

-->66. for(int i=count+Mi-2; i>=0; i--)cout<<c[i];  count er man j vabe pelam
